...
def init_args_parser():
    ...
    parser.add_argument("-d", "--downloadpath", help="downloadpath for the qt5.10.1 source. Default is home/pi/Downloads.",
                        type=str, default=DEFAULT_DOWNLOAD_PATH)
    parser.add_argument("-b", "--buildpath", help="shadow build directory. Default is home/pi/build",
                        type=str, default=DEFAULT_BUILD_PATH)
    parser.add_argument("-j", "--jobs", help="number of jobs for make (j4 in the doc)",
    ...
    parser.add_argument(
        "-a", "--all", help="install all optional development packages (Bluetooth, Audio and gstreamer, ...)", action='store_true')
    parser.add_argument(
        "--bluetooth", help="install optional development packages for bluetooth", action='store_true')
    parser.add_argument(
        "--audio", help="install optional development packages for audio", action='store_true')
    parser.add_argument(
        "--database", help="install optional development packages for databases", action='store_true')
    parser.add_argument(
        "--print", help="install optional development packages for printing", action='store_true')
    parser.add_argument(
        "--wayland", help="install optional development packages for wayland support", action='store_true')
    parser.add_argument(
        "--accessibility", help="install optional development packages for accessibility", action='store_true')
    parser.add_argument(
        "--distupgrade", help="edits sources.list and performes dist-upgrade", action='store_true')
    parser.add_argument(
        "--rpiupdate", help="updates the pi's firmware", action='store_true')
...
def start_install_dependencies(args):
    ...
    subprocess.call(["apt-get", "install", "-y", "libx11-dev", "libxext-dev", "libxfixes-dev", "libxrender-dev", "libxcb1-dev", "libx11-xcb-dev", "libxcb-glx0-dev", "build-essential", "libfontconfig1-dev",
                     "libdbus-1-dev", "libfreetype6-dev", "libicu-dev", "libinput-dev", "libxkbcommon-dev", "libsqlite3-dev", "libssl-dev", "libpng-dev", "libjpeg-dev", "libglib2.0-dev", "libraspberrypi-dev"])
    ...
    subprocess.call(["apt-get", "-y", "install", "libudev-dev", "libinput-dev",
...
def start_install_of_qt(args):
    ...
    os.environ['PKG_CONFIG_SYSROOT_DIR'] = args.downloadpath + \
        "/qt-everywhere-src-5.10.1/"

    subprocess.call([args.downloadpath+"/qt-everywhere-src-5.10.1/configure", "-v", "-opengl", "es2", "-eglfs", "-no-gtk", "-qt-xcb", "-device", args.platform, "-device-option", "CROSS_COMPILE=/usr/bin/", "-opensource", "-confirm-license",
                     "-release", "-reduce-exports", "-nomake", "examples", "-no-compile-examples", "-skip", "qtwayland", "-skip", "qtwebengine", "-no-feature-geoservices_mapboxgl", "-qt-pcre", "-ssl", "-evdev", "-qt-freetype", "-prefix", "/opt/Qt5.10.1"])
    ...
    subprocess.call(["make", "-C", args.downloadpath +
                     "/qt-everywhere-src-5.10.1/qtscript"])
...
def install_audio():
    subprocess.call(["apt-get", "-y", "install", "libasound2-dev", "pulseaudio", "libpulse-dev", "libgstreamer1.0-dev", "libgstreamer-plugins-base1.0-dev", "gstreamer1.0-plugins-base",
                     "gstreamer1.0-plugins-good", "gstreamer1.0-plugins-ugly", "gstreamer1.0-plugins-bad", "gstreamer1.0-pulseaudio", "gstreamer1.0-tools", "gstreamer1.0-alsa", "gstreamer-tools"])
...
def main():
    if not os.geteuid() == 0:
        ...
        if args.distupgrade == True or args.rpiupdate == True:
            if (args.distupgrade == True) and (len(sys.argv) == 2):
                distupgrade()
            elif (args.rpiupdate == True) and (len(sys.argv) == 2):
                rpiupdate()
            else:
                print(
                    "option --distupgrade and --rpiupdate can only be used separately and can't be mixed with other options")
        else:
